...
public class SettingsView extends JPanel {
    ...
    private JScrollPane wrapScrollResponsive(JPanel content) {
        ...
        gbc.fill = GridBagConstraints.HORIZONTAL; // Kéo giãn hết chiều ngang an toàn
        gbc.anchor = GridBagConstraints.NORTH;    // Neo nội dung lên trên cùng
        gbc.insets = new Insets(30, 40, 30, 40);  // Căn lề cực đều
    ...
    private JPanel createTopBar() {
        ...
        right.add(btnRefresh); right.add(btnSave);
    ...
    private JPanel createSidebar() {
        ...
        panel.setPreferredSize(new Dimension(270, 0)); // Sidebar rộng rãi, quyền lực
        ...
        for (int i = 0; i < names.length; i++) {
            ...
            item.addMouseListener(new MouseAdapter() {
                ...
                @Override public void mouseEntered(MouseEvent e) {
                    if (idx != selectedTab) item.setBackground(BG_SIDEBAR_HV);
                }
                @Override public void mouseExited(MouseEvent e) {
                    if (idx != selectedTab) item.setBackground(BG_SIDEBAR);
                }
            });
            panel.add(item);
        }
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // UI HELPERS (Dựng khối giao diện)
    // ══════════════════════════════════════════════════════════════════════════
    ...
    private JPanel fieldBlock(String labelText, JComponent comp) {
        ...
        // FIX CHÍ MẠNG Ở ĐÂY: Set vừa đủ để giãn, không được để Integer.MAX_VALUE
    ...
    private void loadCurrentSettings() {
        ...
        try (Connection con = common.db.DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                switch (key) {
                    case "store_name":    txtStoreName.setText(val); break;
                    case "store_address": txtAddress.setText(val);   break;
                    case "store_phone":   txtPhone.setText(val);     break;
                    case "theme_mode":
                        themeComboBox.setSelectedIndex("Dark".equals(val) ? 1 : 0); break;
                }
            }
        } catch (Exception e) {
    ...
    private void onSave() {
        ...
        if (!newPass.isEmpty()) {
            ...
            if (user != null) {
                try (Connection con = common.db.DatabaseConnection.getConnection()) {
                    String hashed = common.security.PasswordHasher.hash(newPass);
